...
`WorkflowState` 是节点之间唯一的数据契约，包含会话画像、意图、风险、引用、结构化结果、答案来源、请求结果和耗时。风险节点先于检索和生成；`high` 会短路普通 RAG 与结构化查询，答案只保留就医分流。生产请求必须经过用户名归属校验，并以 `request_id + fingerprint` 防止断线重试重复推进。
